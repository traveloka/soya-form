...
import { getFieldNames } from "./_utils";
...
const mapDispatchToProps = (dispatch, props) => ({
  ...
  reorderListItem: fieldNames => (index, targetIndex) => {
    dispatch(
      reorderListItem(props.form.getFormId(), fieldNames, index, targetIndex)
    );
  },
  reorderListItemDec: fieldNames => (index, amount) => {
    dispatch(
      reorderListItemDec(props.form.getFormId(), fieldNames, index, amount)
    );
  },
  reorderListItemInc: fieldNames => (index, amount) => {
    dispatch(
      reorderListItemInc(props.form.getFormId(), fieldNames, index, amount)
    );
  }
});

export const createRepeatableAction = Component =>
  compose(
    withForm,
